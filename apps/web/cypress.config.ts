...
export default defineConfig({
  e2e: {
    ...
    setupNodeEvents(on, config) {
      ...
      config.env.auth0_domain = env.VITE_AUTH0_DOMAIN ?? process.env.VITE_AUTH0_DOMAIN;
      config.env.auth0_audience = env.VITE_AUTH0_AUDIENCE ?? process.env.VITE_AUTH0_AUDIENCE;
      config.env.auth0_client_id = env.VITE_AUTH0_CLIENT_ID ?? process.env.VITE_AUTH0_CLIENT_ID;
