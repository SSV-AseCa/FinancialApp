---
type: story
id: S01.04
feature: [[F01 Authentication]]
tags: [story]
created: 2026-05-06
---

# Log Out

## User Story
As an investor, I want to log out so that my session is securely closed.

## Acceptance Criteria
- The Auth0 session is terminated and the local access token is cleared upon logout
- After logout, protected resources are no longer accessible without re-authentication
- The logout flow is verified end-to-end on web via Cypress
- The logout flow is verified end-to-end on mobile via Appium

## Tasks

- [[T01.04 Core]]
- [[T01.04 Web]]
- [[T01.04 Cypress]]
- [[T01.04 Mobile]]
- [[T01.04 Appium]]

## Notes
