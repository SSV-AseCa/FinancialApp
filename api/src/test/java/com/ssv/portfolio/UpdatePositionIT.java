...
class UpdatePositionIT {
	...
	@Test
	void updatesQuantityAndDateKeepingTheCompany() throws Exception {
		String sub = "auth0|update-position-it-" + UUID.randomUUID();
		Position position = createPosition(sub, "AAPL", 5, LocalDate.of(2024, 1, 1));

		mockMvc.perform(authenticatedPut(sub, position.getId(), 20, "2024-06-01")).andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value(position.getId().toString()))
				.andExpect(jsonPath("$.ticker").value("AAPL")).andExpect(jsonPath("$.quantity").value(20))
	...
	void returns404WhenPositionBelongsToAnotherInvestor() throws Exception {
		...
		mockMvc.perform(authenticatedPut(subOther, position.getId(), 10, "2024-06-01")).andExpect(status().isNotFound())
	...
	void returns404WhenPositionDoesNotExist() throws Exception {
		...
		mockMvc.perform(authenticatedPut(sub, UUID.randomUUID(), 5, "2024-01-15")).andExpect(status().isNotFound())
				.andExpect(jsonPath("$.message").exists());
	}
	...
	void returns400WhenQuantityIsNotPositive() throws Exception {
		...
		Position position = createPosition(sub, "AAPL", 5, LocalDate.of(2024, 1, 1));
		mockMvc.perform(authenticatedPut(sub, position.getId(), 0, "2024-01-15")).andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.message").exists());
	}
	...
	private MockHttpServletRequestBuilder authenticatedPut(String sub, UUID positionId, int qty, String date) {
